...
@section('title', trans('admin.calendar.actions.index'))

@section('body')

    <calendar-listing
        :data="{{ $data->toJson() }}"
        :url="'{{ url('admin/calendars') }}'"
            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ trans('admin.calendar.actions.index') }}
                        <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0" href="{{ url('admin/calendars/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.calendar.actions.create') }}</a>
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block">
                            ...
                            <table class="table table-hover table-listing">
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' :column="'id'">{{ trans('admin.calendar.columns.id') }}</th>
                                        <th is='sortable' :column="'title'">{{ trans('admin.calendar.columns.title') }}</th>
                                        <th is='sortable' :column="'description'">{{ trans('admin.calendar.columns.description') }}</th>
                                        <th is='sortable' :column="'event_time'">{{ trans('admin.calendar.columns.event_time') }}</th>
                                        <th is='sortable' :column="'event_location'">{{ trans('admin.calendar.columns.event_location') }}</th>
                                        <th is='sortable' :column="'link'">{{ trans('admin.calendar.columns.link') }}</th>
                                        <th is='sortable' class="text-center" :column="'published_at'">{{ trans('admin.calendar.columns.published_at') }}</th>
                                        <th is='sortable' :column="'enabled'">{{ trans('admin.calendar.columns.enabled') }}</th>

                                        <th></th>
                                    </tr>
                                    <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="10">
                                            <span class="align-middle font-weight-light text-dark">{{ trans('brackets/admin-ui::admin.listing.selected_items') }} @{{ clickedBulkItemsCount }}.  <a href="#" class="text-primary" @click="onBulkItemsClickedAll('/admin/calendars')" v-if="(clickedBulkItemsCount < pagination.state.total)"> <i class="fa" :class="bulkCheckingAllLoader ? 'fa-spinner' : ''"></i> {{ trans('brackets/admin-ui::admin.listing.check_all_items') }} @{{ pagination.state.total }}</a> <span class="text-primary">|</span> <a
                                                        href="#" class="text-primary" @click="onBulkItemsClickedAllUncheck()">{{ trans('brackets/admin-ui::admin.listing.uncheck_all_items') }}</a>  </span>

                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3" @click="bulkDelete('/admin/calendars/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                                ...
                                <tbody>
                                    ...
                                    <td>@{{ item.id }}</td>
                                        ...
                                        <td>@{{ item.event_time | datetime }}</td>
                                        ...
                                        <td>@{{ item.link }}</td>
                                            ...
                                            <span v-if="item.published_at <= now">
                                                ...
                                                <span v-if="item.published_at > now">
                                                <small>{{ trans('admin.calendar.actions.will_be_published') }}</small><br />
                                                @{{ item.published_at | datetime('DD.MM.YYYY, HH:mm') }}
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary btn-spinner" href="{{ url('admin/calendars/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.calendar.actions.create') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
